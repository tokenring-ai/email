...
const inputSchema = z.object({
  ...
  box: z.string().optional().describe("Email box to search within, defaults to inbox"),
  limit: z.number().int().positive().default(25).optional(),
  unreadOnly: z.boolean().optional(),
});

async function execute({query, box, limit, unreadOnly}: z.output<typeof inputSchema>, agent: Agent) {
  const messages = await agent.requireServiceByType(EmailService).searchMessages({query, box, limit, unreadOnly}, agent);
  return `
