      ...
      (scriptingService: ScriptingService) => {
        ...
        scriptingService.registerFunction("searchEmailMessages", {
          ...
          async execute(
            ...
            const parsedLimit = limit ? Number.parseInt(limit, 10) : undefined;
            const messages = await this.agent
              .requireServiceByType(EmailService)
              .searchMessages(
                {
                  query,
        ...
        scriptingService.registerFunction("createEmailDraft", {
          ...
          async execute(
            ...
            const draft = await this.agent
              .requireServiceByType(EmailService)
              .createDraft({subject, textBody: bodyText, to}, this.agent);
        ...
        scriptingService.registerFunction("sendCurrentEmailDraft", {
          ...
          params: [],
          async execute(this: ScriptingThis): Promise<string> {
            const sent = await this.agent
              .requireServiceByType(EmailService)
              .sendCurrentDraft(this.agent);
