...
export const EmailWatchSchema = z.object({
  markAsRead: z.boolean().default(false).meta({ description: "Mark matched emails as read after processing" } satisfies ConfigFieldMeta),
  unreadOnly: z.boolean().default(false).meta({ description: "Only consider unread emails" } satisfies ConfigFieldMeta),
